...
//axo//////////////////////////////////////////13/08/2013
///  Liberría para recuperar estadíticas del ITS.
require_once 'its_stats.php';
//////////////////////////////////////////////////////////
...
class learnpath {
    ...
    public function __construct($course, $lp_id, $user_id) {
	   ...
	   global $lp_display_order;  //axo/// array for display

        while ($row = Database::fetch_array($res)) {
            $oItem = '';
            $lp_item_id_list[] = $row['id'];

///axo///////////////////////////////lista de elementos
            $lp_display_order[$row['display_order']] = $row['description'];
...
$grade = Student_Exercise_Progress($item['description'], $user_id);

if($grade == 100)
	$image = '<img src="../img/star.png" border="0" align="absmiddle" >';
elseif($grade > 0)
	$image = '<img src="../img/star_half.png" border="0" align="absmiddle" >';
else
	$image = "";
...
//////////////////////////////////////
            ...
            if ($item['type'] == 'quiz') {
                ...
                if ($item['type'] != 'dokeos_chapter' && $item['type'] != 'dokeos_module' && $item['type'] != 'dir') {
                    $html .= "&nbsp;<img id='toc_img_" . $item['id'] . "' src='" . $icon_name[$item['status']] . "' alt='" . substr($item['status'], 0, 1) . "' width='14' />" . str_repeat('&nbsp;', 3) . $image . " ". $grade;
///axo/////////
/// Aquí arriba va la imagen de progreso
...
/////////////////////////////////////////////////////////////////
//Parametros por URL a Flashvars para usuario profesor
/////////////////////////////////////////////////////////////////

        if ($iframe) {
            $return .= '<iframe id="learnpath_preview_frame" frameborder="0" height="550" width="100%" scrolling="auto" src="' . api_get_path(WEB_COURSE_PATH) . $_course['path'] . '/document' . str_replace('%2F', '/', urlencode($row_doc['path'])) . '?' . "id=" . api_get_user_id() . "&" . api_get_cidreq() . '"></iframe>';
        } else {
