...
/// Llamado de función para recuperar estadísticas
$fields = explode("&", api_get_cidreq());
$course_id	= substr($fields[0],7);
$lecciones = Student_General_Progress($course_id, api_get_user_id());
//$lecciones = Student_General_Progress(api_get_cidreq(), api_get_user_id());
//////////////////////////////////////////////////////
...
if (!empty($flat_list)) {
    ...
    foreach ($flat_list as $id => $details) {
        ...
        $my_title = $name;
///axo/////////////////////////////13/06/2013
/// Área de título de lecciones, aquí insertamos avance en ejerc.
/// $lecciones[$name]
$image='<img src="../img/star.png" border="0" align="absmiddle" >';
$image_2='<img src="../img/star_half.png" border="0" align="absmiddle" >';

$stars = $lecciones[$name] / 20;


if(($lecciones[$name] % 20) > 0)
  $half_star = 1;
else
  $half_star = 0;

if($lecciones[$name])
 $progreso = $lecciones[$name]."%";
else
 $progreso = "";
        ...
        $dsp_line =	'<tr align="center" class="'.$oddclass.'">'.
            		'<td align="left" valign="top">'.Display::return_icon('learnpath.png', get_lang('LPName'),'',ICON_SIZE_SMALL).'
                     <a href="'.$url_start_lp.'">' . $my_title . '</a>'. str_repeat('&nbsp;', 10). str_repeat($image, $stars) . str_repeat($image_2, $half_star) . str_repeat('&nbsp;', 5) . $progreso . $session_img .$extra."</td>";
/////////////////////////////////////////////////////////////
        $dsp_desc = '';
        $dsp_export = '';
        $dsp_edit = '';
...
//axo//////////////////////////////////////////////
//Aqí arriba va el ícono del PDF
///////////////////////////////////////////////////
