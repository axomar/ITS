    ...
    <table width="300">
    	<FORM name="formulario" action="javascript:path()">
            ...
            <tr height="2%">
				...
				<td height="2%" align="center"><INPUT hidden TYPE="hidden" NAME="date2" class = "datepicker" id="fecha_2" /></td>
...
<script>
function tomar_fecha_1(){
	var contenido = document.getElementById('fecha_1').value;
	return contenido;
}
</script>
...
//// Conexi�n a la BD e inserci�n de datos //////////////////////
 ...
 if($fecha1=="" or $fecha2 == ""){
   $query  = "select date,count(date) from its_summary where user_id = ".$user_id." and course = '".$course_code."' group by(date)";
 }
 else
 {
    $query  = "select date,count(date) from its_summary where user_id = ".$user_id." and course = '".$course_code."' and date >= '".$fecha_1_des."' and date <= '".$fecha_2_des."' group by(date)";
 }
 $result = mysql_query($query) or die (mysql_error());
 
 $query_tiempo = "select sum(elapsed_time) from its_summary where user_id =".$user_id." and course = '".$course_code."'";

 while($row = mysql_fetch_array($result)){	
 	$query_tiempo = "select sum(elapsed_time) from its_summary where user_id =".$user_id." and course = '".$course_code."' and date='".$row['date']."'";
	...
	$time1 = ($tiempo['sum(elapsed_time)']/3600);
	 ...
	 echo "
	 <input style = 'display:none' value='http://lcc.ens.uabc.mx/~siec/chamilo/main/mySpace/statistics_details.php?student=".$user_id."&course=ITSM2013&origin=tracking_course&cidReq=ITSM2013&id_session=0' id ='URL_Hide' />
		...
		<tr>
			<tr class = 'clickable' id ='t_".$row['date']."'>
				...
				<tr>
						<td  width=\"5%\" align='center'> <b>Hora de acceso:</b> </td>
		...
		";
	  $query1 = "select time,exercise_id,hints,errors,result,elapsed_time from its_summary where (user_id =".$user_id.") and (date = '".$row['date']."')";
	  
 	  $resultado = mysql_query($query1) or die (mysql_error());
	  while($brawl = mysql_fetch_array($resultado)){
		 echo "
				<tr>   
						<td width=\"5%\" >  ".$brawl['time']." </td>
...
/*
	$('img').on({	
		'click': function() {
			var src = ($(this).attr('src') === '../img/icons/22/zoom_in.png')
				? '../img/icons/22/zoom_out.png'
				: '../img/icons/22/zoom_in.png';
			$(this).attr('src', src);
			
			$fecha = document.getElementById('2013-08-15');
			alert($fecha);
			$fecha.nextUntil('tr.clickable').slideToggle();
			}
	});
	$(".clickable").click(function() {
		$(this).nextUntil('tr.clickable').slideToggle();
		$fecha = document.getElementById('i_2013-08-15');
		//alert($('#i_2013-08-15').attr('src'));

			var src = ($('#i_2013-08-15').attr('src') === '../img/icons/22/zoom_in.png')
				? '../img/icons/22/zoom_out.png'
				: '../img/icons/22/zoom_in.png';
			$('#i_2013-08-15').attr('src', src);
...
$(function () {
    $('#contenedor2').highcharts({
        ...
        title: {
            text: 'Tiempo del alumno en el sistema'
        },
        yAxis: {
            allowDecimals: false,
            title: {
...
<div id="container-9">
	...
	<script src="http://code.highcharts.com/modules/data.js"></script>
        ...
        <?php
		 ...
		 if($fecha1=="" or $fecha2 == ""){
			$query_grafica = "select date,count(exercise_id) from its_summary where user_id = ".$user_id." and course = '".$course_code."' group by(date)";
			 }
		else
			 {
			 $query_grafica = "select date,count(exercise_id) from its_summary where user_id = ".$user_id." and course = '".$course_code."' and date >= '".$fecha_1_des."' and date <= '".$fecha_2_des."' group by(date)";
			 }
        ...
        <li>
		 ...
		 if($fecha1=="" or $fecha2 == ""){
			$query_grafica = "select date,sum(elapsed_time) from its_summary where user_id = ".$user_id." and course = '".$course_code."' group by(date)";
			 }
		 else
			 {
		    $query_grafica = "select date,sum(elapsed_time) from its_summary where user_id = ".$user_id." and course = '".$course_code."' and date >= '".$fecha_1_des."' and date <= '".$fecha_2_des."' group by(date)";
			 }
