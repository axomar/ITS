...
var app = angular.module('lordAngeloApp', []);
    ...
    $scope.hola = function() {
        ...
        var myselect = "select  " +
            "ng-options=\"item for item in choices[myid].values\""+
            "ng-class=\"choices[myid].resp\" " +
            "ng-blur=\"getSolutions()\" " +
            "id=\"{{choices[myid].id}}\" " +
            "ng-model=\"choices[myid].value\" " +
            "name=\"{{choices[myid].name}}\" " +
        ...
        var res = res.replace(/InputSelect/g, function myFunction(x){
            var mystringID = myselect.replace(/myid/g, function myFunction(x){return $scope.myid});
            $scope.choices.push({name:"name"+$scope.myid,id:$scope.myid,type:'select',value:'',values:['uno','dos']});
            $scope.myid++;
            return "\\html'&lt;"+mystringID+"'"
